...
###test code on small table###

###########################
#####old stuff: ignore#####
###########################

# basePath <- "./" #wd is already Project
# fname <- "smalltable.txt"
# smalltbldf <- read.table(paste0(basePath, fname), sep = "", header = T)

#make molten tidy df: melt 3 measured vars into a single column, add a value column
# dfMelt <- melt(smalltbldf, id = c("subj","activity"), measure.vars = names(smalltbldf)[3:5])

#apply dcast to get a df of means
# meanData <- dcast(dfMelt, subj + activity ~ variable, mean)
# meanData
# names(meanData)[3:5] <- paste0(names(meanData)[3:5], ".Variable.Mean")
...
#total of 134 columns. The subject number and activity are 
#columns 1 and 2, resp. After that, there will be one column for the mean of each of the 66 physical
#variables and one column for the standard deviation of each of those variables.
#There will be one row for each combination of subject and activity.

# test <- split(mergedData_Mean_StdDev, mergedData_Mean_StdDev$subjectnumber)
# test <- lapply(test, split, mergedData_Mean_StdDev$activity)
# mylen <- 0
# for(i in 1:length(test)){
   # mylen <- mylen + length(test[[i]])
# }

# res <- list(numeric(5), character(5), rep(numeric(5),3))
# names(res) <- c("x","y","a","b","c")

# f1 <- function (nRow) 
# {
    # data <- as.environment(list(x = numeric(nRow), y = numeric(nRow), 
        # z = numeric(nRow)))
...
# system.time(r1 <- f1(mylen))

# for(i in 1:length(l)){
	# for(j in 1:length(l[[i]])){
        # subj <- l[[i]][[j]]$c1[1]
		# activ <- l[[i]][[j]]$c2[1]
		# means <- lapply(l[[i]][[j]][, 3:5], mean)
		# stdevs <- lapply(l[[i]][[j]][, 3:5], sd)
		# rowvec <- c(subj, activ, means, stdevs)
		# names(rowvec) <- c("subject", "activity", "mean1","mean2","mean3", "sd1","sd2","sd3")
		# finaldf <- rbind(finaldf, rowvec)
	# }
# }



# df1 <- data.frame(c1 = rnorm(20), c2 = 1:5, c3 = c("a1","a2","a3","a4"))
# spdf1 <- split(df1, df1$c2) #list of 5 dfs
# spdf1 <- lapply(spdf1, split, df1$c3) #list of 5 lists, each of which is a list of 4 dfs


# df1 <- data.frame(c1 = rep(1,3), c2 = c("walk","walk","walk"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df2 <- data.frame(c1 = rep(2,3), c2 = c("run","run","run"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df3 <- data.frame(c1 = rep(3,3), c2 = c("jump","jump","jump"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df4 <- data.frame(c1 = rep(4,3), c2 = c("climb","climb","climb"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df5 <- data.frame(c1 = rep(5,3), c2 = c("swim","swim","swim"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df6 <- data.frame(c1 = rep(6,3), c2 = c("bike","bike","bike"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df7 <- data.frame(c1 = rep(7,3), c2 = c("run","run","run"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df8 <- data.frame(c1 = rep(8,3), c2 = c("swim","swim","swim"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df9 <- data.frame(c1 = rep(9,3), c2 = c("row","row","row"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# df10 <- data.frame(c1 = rep(10,3), c2 = c("row","row","row"), c3 = rnorm(3), c4 = rpois(3,5), c5 = rexp(3))
# l <- list(list(df1,df2,df3,df4), list(df5), list(df6, df7,df8,df9, df10))
